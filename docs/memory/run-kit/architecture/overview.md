...
HexoKit is a web-based agent orchestration dashboard. Product identity: **HexoKit** is the product name; `rk` is the binary; `run-kit` is the still-installed long command, roster, and formula name until the rebrand plan's R0–R2 rows (`fab/plans/sahil/26-09-10-hexokit-rebrand.md`), with everything substrate-coupled (`RK_*` env, `@rk_*` options, `rk-*` names, `~/.config/run-kit/` paths, the `docs/memory/run-kit/` domain) staying `run-kit`/`rk` by design (260911-mljj-hexokit-brand-prose). In production, a single Go binary runs as a daemon in a dedicated tmux session:
...
- Web-tile forward proxy: the daemon doubles as an HTTP forward proxy on its listen port (CONNECT tunnels + absolute-form plain-HTTP forwarding, handled by a wrapper ahead of the router — the second transport-scoped §IX exception beside `/mcp`, no route added, CORS allowlist unchanged) with **no destination restriction** — anyone who can reach rk already has a shell on the host through the terminal relay (no auth; Tailnet-only / SSH-tunnel-only by deployment), so a destination policy would add no protection, and browser pages cannot emit CONNECT or absolute-form request lines, so no CSRF surface opens. The proxy dials with `net.Dialer` timeouts, spawns no subprocess (§I), and holds no state beyond live connections (§II) — see [api-and-sockets](/run-kit/api-and-sockets.md) § Forward Proxy and [desktop-shell](/run-kit/desktop-shell.md) § Design Decisions → No destination policy on the forward proxy
